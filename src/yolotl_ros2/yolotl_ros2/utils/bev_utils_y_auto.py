        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            if len(self.src_points) < self.max_points:
                ...
                if current_point_index == 1:   # Right-Bottom 선택 시
                    y_bottom = self.src_points[0][1] # Left-Bottom의 y좌표 가져오기
                    final_point = (x, y_bottom)
                elif current_point_index == 3: # Right-Top 선택 시
                    y_top = self.src_points[2][1]    # Left-Top의 y좌표 가져오기
                    final_point = (x, y_top)

                self.src_points.append(final_point)
                print(f"[INFO] Added {point_order[current_point_index]} point: {final_point} ({len(self.src_points)}/{self.max_points})")
                ...
                if len(self.src_points) < 4:
                    ...
                    try:
                        with open(self.args.out_txt, 'w') as f:
                            f.write("# Selected BEV Points (Y-Auto)\n")
                            for i, pt in enumerate(self.src_points):
